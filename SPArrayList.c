...
SPArrayList* spArrayListCreate(int maxSize)
{
    ...
    arr->elements = malloc(maxSize*sizeof(int));
...
SPArrayList* spArrayListCopy(SPArrayList* src)
{
    ...
    memcpy(arr->elements, src->elements, src->actualSize*sizeof(int));
    return arr;
}
...
SP_ARRAY_LIST_MESSAGE spArrayListAddAt(SPArrayList* src, int elem, int index) {
    ...
    if (index == src->actualSize) {
        ...
        memmove(&src->elements[index+1], &src->elements[index], (src->actualSize-index)*sizeof(int));
        src->elements[index] = elem;
    }
    src->actualSize++;
    return SP_ARRAY_LIST_SUCCESS;
...
SP_ARRAY_LIST_MESSAGE spArrayListRemoveAt(SPArrayList* src, int index){
    ...
    if (index != src->actualSize-1) {
        memmove(&src->elements[index], &src->elements[index+1], (src->actualSize-index-1)*sizeof(int));
    }
    src->actualSize--;
    return SP_ARRAY_LIST_SUCCESS;
}
